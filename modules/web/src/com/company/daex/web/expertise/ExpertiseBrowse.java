...
public class ExpertiseBrowse extends AbstractLookup {
    ...
    {
        Map<String, Object> reportParams = new HashMap<>();
        reportParams.put("entities", expertisesTable.getSelected());

        LoadContext<Report> lContext = new LoadContext<>(Report.class);
        lContext.setQueryString("select r from report$Report r where r.code = 'Report_3' ");

        Report report = dataManager.load(lContext);
        reportGuiManager.printReport(report, reportParams, "DEFAULT", "Report_3");
    }
    ...
    public void Report_4()
    {
        ...
        LoadContext<Report> lContext = new LoadContext<>(Report.class);
        lContext.setQueryString("select r from report$Report r where r.code = 'Report_4' ");

        Report report = dataManager.load(lContext);
        reportGuiManager.printReport(report, reportParams, "DEFAULT", "Report_4");
    }
